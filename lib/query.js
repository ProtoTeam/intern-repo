...
var Query = /** @class */ (function () {
    ...
    Query.prototype.select = function () {
        ...
        lodash_1.map(fields, function (field) {
            // 找出计算值
    ...
    Query.prototype.caculate = function (data, field, operator) {
        switch (operator) {
            case 'sum':
                return {
                    field: lodash_1.sumBy(data, field),
                };
            case 'min':
                return lodash_1.minBy(data, field);
            case 'max':
                return lodash_1.maxBy(data, field);
        }
    };
    /**
     * 返回最后的查询数据
    ...
    Query.prototype.record = function () {
        ...
        // 处理分组
        if (this._groupby)
            this._groupColumn = lodash_1.groupBy(this.data, this._groupby);
        // 处理求值 max min sum
        if (this._caculate) {
            // max
            ...
            if (this._groupColumn) {
                this.data = [];
                lodash_1.forIn(this._groupColumn, function (item) {
        ...
        //处理排序
        if (this._orderby)
            this.data = lodash_1.orderBy(this.data, this._orderby.fields, this._orderby.asc);
        //处理 limit
        if (this._limit)
            this.data = this.data.slice(0, this._limit);
        //处理select ['id', 'score']
        if (!lodash_1.isEmpty(this._select)) {
            this.data = lodash_1.map(this.data, function (item) { return lodash_1.pick(item, _this._select); });
        }
        return this.data;
    };
    return Query;
